...
import { Calendar, Star, Smile, HelpCircle } from 'lucide-react';
    ...
    <Layout>
      <div className="space-y-6">
        <section className="space-y-4">
          <h1 className="text-3xl font-bold">{t('language') === 'de' ? 'Willkommen bei BookRadar' : 'Welcome to BookRadar'}</h1>
          <p className="text-muted-foreground">
            {t('language') === 'de' 
              ? 'Entdecke neue Bücher, verfolge deinen Leseprozess und finde Empfehlungen basierend auf deinen Interessen.'
              : 'Discover new books, track your reading progress, and find recommendations based on your interests.'}
          </p>
        </section>

        <section className="space-y-4">
          <h2 className="text-2xl font-semibold">{t('language') === 'de' ? 'Funktionen' : 'Features'}</h2>
          {renderFeatureCards()}
        </section>
      </div>
    </Layout>
